<template>
    <section id="section-imagesAnime">
        <h2 class="tituloImg">{{title}}</h2>
        <div>
           
            <div id="selectSlide">
                <div class="slideOne">         
                    <input @click="functionselectedItem1()" type="radio" class="btn-check slide-controller2"  autocomplete="off" :name="nameSlide" :id="nameSlide+'1'" checked />
                    <label class="btn btn-outline-danger opcao1" :for="nameSlide+'1'">
                ...
                <div class="slideTwo">
                    <input @click="functionselectedItem2()" type="radio" class="opcao2 btn-check slide-controller"  autocomplete="off" :name="nameSlide"  :id="nameSlide+'2'"  />
                    <label class="btn btn-outline-danger" :for="nameSlide+'2'">
                    ...
                    <div class="slides-list teste " :class="{selectedItem1: selectedItem1, selectedItem2: selectedItem2}"  > 
                        <ul class="ul-lista">
                            <li class="slide li-lista" v-for="(fotoBloco1) in firstbloco" :key="fotoBloco1">
                                <a :href="'tv/id/'+fotoBloco1.id"><img class="img-lista" :src="fotoBloco1.path"></a>
                                <router-link :to="rotas" :id="fotoBloco1.id"></router-link>
                            </li>
                        </ul>
                        <ul>
                            <li class="slide li-lista" v-for="(fotoBloco2) in secondbloco" :key="fotoBloco2">
                                <a :href="'tv/id/'+fotoBloco2.id"> <img class="img-lista" :src="fotoBloco2.path"></a>                   
                            </li>
                        </ul>
                    </div>
                </div>
...
<script>
export default{
    props: ['firstbloco', 'secondbloco', 'nameSlide', 'title'],
    data(){
        return{
            selectedItem1: true,
            selectedItem2: false,
